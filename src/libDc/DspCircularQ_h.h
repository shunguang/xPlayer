...
*/
//----------------------------------------------------------------------------------------
#ifndef _DSP_CIRCULAR_Q_H_H_
#define _DSP_CIRCULAR_Q_H_H_

#include "DspFrm_h.h"
namespace xPlayer {
	class  DspCircularQ_h {
	public:
		DspCircularQ_h( const uint32_t imgW0=32, const uint32_t imgH0=32, const uint32_t nTotItems=10, const uint32_t L=0, const std::string &name="RgbFrmQue");
		~DspCircularQ_h();
		...
		bool wrt(const DspFrm_h *src);  	//host interface
		bool read(DspFrm_h *dst);       	//host interface 
	...
	typedef std::shared_ptr<DspCircularQ_h> DspCircularQ_hPtr;
}
