...
_Tp xPlayer::sumVal(const cv::Mat &x)
{
	_Tp s = 0;
...
//return a col vector
Mat xPlayer::sumCols(const cv::Mat &x)
{
	//cout<<x<<endl;
	int n = x.cols;
...
//return a row vector
Mat xPlayer::sumRows(const cv::Mat &x)
{
	//cout<<x<<endl;
...
void xPlayer::gray2bgr(const cv::Mat &gray, cv::Mat &bgr)
{
	vector<Mat> ch;
	ch.push_back(gray);
	ch.push_back(gray);
...
void xPlayer::convert_to_CV_8U(const cv::Mat &in, cv::Mat &out)
{
...
bool xPlayer::myCvInv(const cv::Mat &X, cv::Mat &invX, double &det)
{
	det = determinant(X);
	if (det < APP_EPS2) {
		return false;
...
float xPlayer::maskNonZeroPercent(const Mat &mask, const int &xo, const int &yo, const int &xb, const int &yb, size_t &n)
{
#if 1
...
void xPlayer::rectPyrLevelUpDown(const cv::Rect &in, const int dL, cv::Rect &out)
{
	if (dL == 0) {
		out = in;
		return;
...
void xPlayer::setCvImg(IplImage *des, const int &desRoiXo, const int &desRoiYo,
...
bool xPlayer::getCvImgRowStep(const IplImage *img, int &rowStep, int &row0)
{
...
std::string xPlayer::genImgFilePath(const string &myPath, const string &fileNameWoExt, const uint64_t fn, const int L)
{
	char fileName[1024];
...
void xPlayer::myImgWrite(const cv::Mat &x, const std::string &dir, const std::string &fname, const size_t fn, const size_t L)
{
...
void xPlayer::myImgWrite(const IplImage* cvTrkImg, const string &myPath, const string &fileNameWoExt, const size_t &fn)
{
	std::string fileName = genImgFilePath(myPath, fileNameWoExt, fn);
	if (!folderExists(myPath)) {
		printf("xPlayer::myCvImgSave(): Path: %s is not exist!\n", myPath.c_str());
		printf("xPlayer::myCvImgSave(): could not save file: %s\n", fileName.c_str());
		return;
	}

	if (!cvSaveImage(fileName.c_str(), cvTrkImg)) {
		printf("xPlayer::myCvImgSave():  Could not save: %s\n", fileName.c_str());
	}
	else {
		printf("xPlayer::myCvImgSave():  File saved: %s\n", fileName.c_str());
	}
}

void xPlayer::dumpCvImgToTextFile(const IplImage *J, const string &fileName)
{
	int w = J->width;
	int h = J->height;
...
void xPlayer::drawPlus(cv::Mat &img, const cv::Point2f &pt, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
	Point p(pt.x, pt.y);
	drawPlus(img, p, len, color, thickness, lineType, shift);
}

void xPlayer::drawPlus(Mat &img, const Point &pt, const uint32_t len, const Scalar& color, int thickness, int lineType, int shift)
{
...
void xPlayer::drawPluses(cv::Mat &img, const std::vector<cv::Point2f> &v, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
...
void xPlayer::drawPluses(cv::Mat &img, const std::vector<cv::Point> &v, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
...
void xPlayer::wrtieStrings(Mat& img, const vector<string>& vStrs, const cv::Point &org, int fontFace, double fontScale, cv::Scalar color, int thickness, int lineType)
{
...
void xPlayer::writeFrmNum(cv::Mat &img, const uint64_t frmNum, const cv::Point &org )
{
	cv::putText(img, std::to_string(frmNum), org, cv::FONT_HERSHEY_DUPLEX, 1, cv::Scalar(0,143,143), 2);
}

void xPlayer::drawRect(cv::Mat &img, const Roi &rect, const uint32_t len, const cv::Scalar& color, int thickness, int lineType, int shift)
{
	//LT
